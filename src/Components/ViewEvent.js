...
class ViewEvent extends Component {
    ...
    render() {
        const { event, loading } = this.props;
            ...
            return (
                <div className={`ui container raised very padded segment ${loading ? 'loading' : ''}`} style={{marginTop: "8vh", width: "900px"}}>
                    ...
                    <div className="ui items">
                        <div className="item">
                            {event && <div className="content">
                                ...
                                <div className="extra"><i className="icon marker"></i>{event.where.split("$")[1]}</div>
                                <div className="description">{event.description}</div>
                                <div className="extra">
                                    <EventRegister eventId={event.id} comments={event.comments} />
                                </div>
                            </div>}
                        </div>
                    </div>
